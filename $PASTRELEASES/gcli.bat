...
if not exist "%TARGET%\node_modules\@google\gemini-cli" (
    ...
    :: Added the --ignore-scripts flag to prevent native build failures
    call "%NPM_CMD%" install @google/gemini-cli --prefix "%TARGET%" --no-save --no-audit --no-fund --quiet --ignore-scripts
)

:: 6. Direct Binary Execution
:: Point to the autogenerated .cmd wrapper instead of a hardcoded JS file
...
if exist "%ENTRY%" (
    ...
    echo [FATAL] Install failed to produce the executable.
