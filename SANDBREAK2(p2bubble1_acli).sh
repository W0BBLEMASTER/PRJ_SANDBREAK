...
cat << 'PYEOF' > "$FB_BIN/out_filter.py"
import sys, os, re

# Matches Kitty Protocol queries (e.g. \x1b[?u, \x1b[>1u, \x1b[=1;1u)
# and Primary Device Attributes (\x1b[c)
pattern = re.compile(b'\x1b\\[(?:[=?>][0-9;]*[uc]|c)')

# Matches incomplete sequences at the very end of the buffer
incomplete_pattern = re.compile(b'\x1b\\[[^\x40-\x7e]*$|\x1b$')
...
try:
    while True:
        data = os.read(0, 8192)
        if not data:
            if buf: os.write(1, buf)
            break
        buf += data
        
        match = incomplete_pattern.search(buf)
        if match:
            split_idx = match.start()
            process_data = buf[:split_idx]
            buf = buf[split_idx:]
        else:
            process_data = buf
            buf = b""
        ...
        if process_data:
            clean_data = pattern.sub(b"", process_data)
            os.write(1, clean_data)
except OSError:
    pass
PYEOF

cat << 'ACLIEOF' > "$FB_BIN/acli"
#!/bin/bash
...
for i in {1..10}; do
    script -q /dev/null -e -c "LD_PRELOAD=\"$FB_BIN_DIR/sandbreak.so\" \"$FB_BIN_DIR/agy\" $@" | python3 "$FB_BIN_DIR/out_filter.py"
