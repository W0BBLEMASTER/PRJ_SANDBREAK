...
:: Configuration (Exposed for DNS/VPN resolution issues)
set "INSTALL_URL=https://antigravity.google/cli/install.ps1"
...
powershell.exe -NoProfile -Command "Invoke-WebRequest -UseBasicParsing -Uri '%INSTALL_URL%' -OutFile '%SANDBOX_ROOT%\appdata\install.ps1'" || (
    echo [ACLI-FATAL] Network or download failure detected.
    echo [ACLI-FATAL] Domain not found. Ensure you are on VPN, check system proxies, or update INSTALL_URL.
    pause
    exit /b
)
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%SANDBOX_ROOT%\appdata\install.ps1" --skip-path --skip-aliases || (
    echo [ACLI-FATAL] Powershell payload staging failed.
    pause
    exit /b
)
...
if exist "%AGY_EXE%" (
    "%AGY_EXE%" %* || (
        echo [ACLI-FATAL] agy.exe crashed or returned an error code.
        pause
    )
) else (
