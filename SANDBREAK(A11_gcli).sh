#!/bin/bash
# A11.sh: Fakebox (gcli)
# Exact A10-2 Baseline + Type-Safe Input Fix + BROWSER=echo
...
if command -v gcc >/dev/null 2>&1; then
  ...
  echo "[Warning] gcc not found, skipping sandbreak.so compilation."
fi
...
if command -v yarn >/dev/null 2>&1; then
  ...
  echo "[Warning] yarn not found, skipping @google/gemini-cli install."
fi
...
if [ ! -w "$BIN_DIR" ]; then
  if [ -n "$PREFIX" ] && [ -w "$PREFIX/bin" ]; then
    ...
    BIN_DIR="$HOME/FAKEBOX/bin"
  fi
fi

sudo tee "$BIN_DIR/gcli" > /dev/null << 'EOF'
#!/bin/bash
YARN_BIN_DIR=$(yarn global bin 2>/dev/null || echo "/usr/local/bin")
...
EOF
sudo chmod +x "$BIN_DIR/gcli"
echo "A11 (Fakebox) Deployed."
