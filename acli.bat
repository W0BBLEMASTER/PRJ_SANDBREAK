...
powershell.exe -NoProfile -Command "Invoke-WebRequest -UseBasicParsing -Uri 'https://antigravity.google/cli/install.ps1' -OutFile '%SANDBOX_ROOT%\appdata\install.ps1'"

echo [ACLI] Executing payload staging (Bypassing PATH and Aliases)...
powershell.exe -NonInteractive -NoProfile -ExecutionPolicy Bypass -File "%SANDBOX_ROOT%\appdata\install.ps1" --skip-path --skip-aliases
