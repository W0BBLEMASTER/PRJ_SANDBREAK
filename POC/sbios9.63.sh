...
void patch_unsupported_syscalls(uint8_t *data, size_t size) {
    for (size_t off = 0; off < size - 16; off += 4) {
        uint32_t inst = *(uint32_t*)(data + off);
        if (inst == 0xD2803648 || inst == 0xD2803668) {
            for (size_t j = 4; j <= 16; j += 4) {
                if (*(uint32_t*)(data + off + j) == 0xD4000001) {
                    *(uint32_t*)(data + off + j) = 0x928004A0;
                    printf("[*] Strategy C: Patched SVC #0 for unsupported syscall at offset %zu\n", off + j);
                    break;
                }
            }
        }
...
int main(int argc, char **argv) {
    ...
    patch_unsupported_syscalls(data, size);
...
qemu-aarch64 -strace -E LD_PRELOAD="$HOME/FAKEBOX/bin/sandbreak.so" -L "$HOME/FAKEBOX/sysroot" "$HOME/FAKEBOX/patched_agy" "$@"
EOF_ACLI
